...
$(function() {
    ...
    $(document).on('click', '.select-object', function() {
    	...
    	.on('dblclick', function(d) {
    		clearTimeout(timeout);
            if (d.target.__data__) {// click on rect
                var node_url =  url_term.replace("replace_me", d.target.__data__.name);
                var win = window.open(node_url, '_blank');
                win.focus();
                // window.alert(d.target.__data__.name)
    		}
    	})
    ...
    var timeoutID;
		...
		function waitForGraphData() {
		    if ($("#rootG").length) {
		        ...
		        timeoutID = setTimeout(waitForGraphData, 1000);
		    }
		}
...
function drawGraph() {
    ...
    for (var name in graph.data) {
        ...
        for (var depIndex in obj.depends) {
            var link = {
                source : graph.data[obj.depends[depIndex].name],
                target : obj,
                id		 : 'link-' + linkIdCounter++,
                label	 : obj.depends[depIndex].link
            };
    ...
    graph.nodeValues = Object.values(graph.data);
  	...
  	graph.forceSim = d3.forceSimulation()
		...
		if (config.graph.preventColls == "false") {
				graph.forceSim
					.force("collide", d3.forceCollide().strength(1).radius(config.graph.collide));
		}
    ...
    zoom = d3.zoom()
	  ...
	  /* Possible zoom for nddes needs to get scripted well! */
	  //graph.rootG
    ...
    graph.line = graph.rootG.append('g')
    		.attr("id", "paths")
    		.selectAll('.link')
    ...
    graph.drag = d3.drag()
        ...
        .on('drag', function(evemt, d) {
            ...
            d.y = event.y;
            if (dragged(d)) {
                if (graph.forceSim.alpha() < 0.01) {
                    graph.forceSim.alpha(.25);
    ...
    $('#graph-container').on('click', function(e, d) {
    ...
    const tip = d3.tip()
    	.attr('class', 'd3-tip')
    	.html(function(event, d) { return (d.name) ? d.name : "-"; });
    ...
    setTimeout(function() {
        ...
        graph.preventCollisions = (config.graph.preventColls == "true");
        ...
        .attr('d', function (d) {
           ...
           d3.select(this)
           		.each(function(e) {
				          ...
				          var x    = e.target.x,
				              y    = e.target.y,
				              line = new geo.LineSegment(e.source.x, e.source.y, x, y);

				          for (var f in e.target.edge) {
				              var ix = line.intersect(e.target.edge[f].offset(x, y));
				              if (ix.in1 && ix.in2) {
				                  x = ix.x;
				                  y = ix.y;
				                  break;
...
function selectObject(obj, el) {
    ...
    if (obj.docs.length == 0) { // create HTML documentation for a node only on demand
    	var term_url = url_term.replace("replace_me",obj.name);
        obj.docs = "<h2><a target='_blank' href='" + term_url + "'>" + obj.name + "</a> (" + obj.desc + ")" + "</h2>";
    	// obj.docs += "<p>" + obj.desc + "</p>";
    	if (obj.depends.length > 0) {
    		obj.docs += "<h3>Parents:</h3>";
    		for (i = 0; i < obj.depends.length; i++)
    		{
	    		var parent_url = url_term.replace("replace_me",obj.depends[i].name);
    		    obj.docs += "<p><a target='_blank' href='" + parent_url + "' >" + obj.depends[i].name + "</a> (" + obj.depends[i].desc + ")" + "</p>";
    		}
    	}
    	if (obj.dependedOnBy.length > 0) {
    		obj.docs += "<h3>Children:</h3>";
    		for (i = 0; i < obj.dependedOnBy.length; i++)
    		{
	    		var child_url = url_term.replace("replace_me",obj.dependedOnBy[i].name);
    		    obj.docs += "<p><a target='_blank' href='" + child_url + "'>" + obj.dependedOnBy[i].name + "</a> (" + obj.dependedOnBy[i].desc + ")" + "</p>";
    		}
    	}
    };
    $('#docs').html(obj.docs);
...
function updateLinkLabels() {
  ...
  graph.linkLabel = d3.select("#paths").selectAll('.link-label')
     .data(graph.links)
  ...
  // add a textPath element for each label to display
     ...
     .attr('transform', function(d) {
     		if (d.source.x > d.target.x) {
     			return "rotate(180," + [(d.target.x + (d.source.x - d.target.x)/2), (d.target.y + (d.source.y - d.target.y)/2)] + ")"
     			//return "translate(0,0)rotate(180)"
     		}
     		else { return "translate(0,0)"}
     })
     //.attr('transform-origin', function(d) { return Math.abs((d.source.x - d.target.x)/2) + "," + Math.abs((d.source.y - d.target.y)/2) })
     */
  graph.linkLabel
     .append('textPath')
        ...
        if (obj !== highlighted) {
            ...
            graph.node.classed('inactive', function(d) {
                return (obj !== d
                     ...
                     && d.depends.map(function(e) { return e.name; }).indexOf(obj.name) == -1
                     //&& d.dependedOnBy.indexOf(obj.name) == -1);
                     && d.dependedOnBy.map(function(e) { return e.name; }).indexOf(obj.name) == -1);
            });
            ...
            graph.linkLabel.classed('inactive', function(d) {
        		...
        		//graph.links.filter(link => (obj == link.source) || (obj == link.target)).forEach(el => highLabels.push(el));
            //updateLinkLabels(highLabels);
        }
        highlighted = obj;
    } else {
        if (highlighted) {
...
function zoomFit(paddingPercent, transitionDuration) {
	...
	var parent = rootG.node().parentElement.parentElement;
