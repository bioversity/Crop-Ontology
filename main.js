...
apejs.urls = {
		...
		get: function(request, response) {
			...
			if(!Common.isblank(token)) {
				try {
					...
					Common.print(response).text(ibpHtml);
				} catch(e) {
				}
			}
		}
	...
	"/api": {
			get: function(request, response) {
					var html = Common.renderIndex("skins/api.html", {URL:"http://www.cropontology.org" });
					Common.print(response).text(html);
			}
	},
	"/about": {
			get: function(request, response) {
					var html = Common.renderIndex("skins/about.html");
					Common.print(response).text(html);
			}
	},
	"/video-tutorials": {
			get: function(request, response) {
					var html = Common.renderIndex("skins/video-tutorials.html");
					Common.print(response).text(html);
			}
	},
	"/help": {
			get: function(request, response) {
					var html = Common.renderIndex("skins/help.html");
					Common.print(response).text(html);
			}
	},
	"/latest": {
			get: function(request, response) {
					var html = Common.renderIndex("skins/latest.html");
					Common.print(response).text(html);
			},
	...
	"/ontologies": {
			get: function(request, response) {
					...
					Common.print(response).json(res, request.getParameter("callback"));
	...
	"/ontology(?:/([^/]*)(?:/([^/]*)(?:/([^/]*))?)?)?": {
			...
			get: function(request, response, matches) {
					...
					if(matches[3] && matches[3] == "rss") {
							...
							// do rss comments
							/*
							Common.print(response).json(ontoComments);
							return;
							*/
							response.setContentType("application/xml");
							Common.print(response).rss(matches[2] + " ("+matches[1]+") - Latest Comments", ontoComments);
					...
					if(matches[3] && (matches[3] == "nt" || matches[3] == "rdf")) {
						 ...
						 // response.setHeader("contentType", "text/plain");
							...
							if(ontologyId == "CO_320"){
									...
									var result = httpget(url);
									Common.print(response).text(result);
					...
					var html = Common.renderIndex("skins/onto.html", {
						URL:"http://www.cropontology.org",
	...
	"/get-ontology-roots/([^/]*)": {
		get: function(request, response, matches) {
			...
			try {
				...
				Common.print(response).json(ret, request.getParameter("callback"));
			} catch (e) {
				response.sendError(response.SC_BAD_REQUEST, e);
			}
		}
	...
	"/get-ontology-id": {
			get: function(request, response) {
					...
					if(Common.isblank(ontology_name))
							return Common.error(response, "Missing ontology_name");
					...
					if(!ontology)
							return Common.error(response, "No traits found for this ontology crop");
			...
			post: function(request, response) {
					try {
							...
							if(!ontologyId || ontologyId.equals(""))
									return Common.error(response, "Missing required parameter");

							var currUser = auth.getUser(request);
							if(!currUser)
									return Common.error(response, "Not logged in");
							...
							if(!auth.isAdmin(currUser)) {
									if(!ontoEntity.getProperty("user_key").equals(currUser.getKey()))
											return Common.error(response, "You don't have the permissions to delete this ontology");
							}
			...
			get: function(request, response, matches) {
					...
					try {
							...
							children.forEach(function(term) {
									...
									var relationship = Common.defaultRelationship(term.getProperty("relationship"));
	...
	"/get-attributes/([^/]*)/jsonrdf": {
			get: function(request, response, matches) {
					...
					Common.print(response).json(object, request.getParameter("callback"));
			}
	},
	"/get-attributes/(.*)": {
			get: function(request, response, matches) {
					...
					Common.print(response).json(attributes, request.getParameter("callback"));
			}
	},
	"/add-attribute": {
			get: function(){},
	...
	"/obo/([^/]*)": {
			get: function(request, response, matches) {
					...
					Common.print(response).textPlain('format-version: 1.2');
					Common.print(response).textPlain('ontology: '+crop+'_ontology');
					Common.print(response).textPlain('');
					...
					select('term')
							...
							.each(function() {
									...
									Common.print(response).textPlain('[Term]');

									var r = new rdf();
									Common.print(response).textPlain('id: ' + r.encodeID(this.id));
									Common.print(response).textPlain('name: ' + Common.translate(this.name, isoLang));
									//definitions
									if(this['Description of Trait'] ) {
											if(this['Crop']){
													Common.print(response).textPlain('namespace: ' + Common.translate(this['Crop'], isoLang)+'Trait');
											}else if (crop!=''){
													Common.print(response).textPlain('namespace: ' + crop+'Trait');
											}
											Common.print(response).textPlain('def: "' + Common.translate(this['Description of Trait'], isoLang) + '" []');
											...
											Common.print(response).textPlain('def: "' + Common.translate(this['Trait description'], isoLang) + '" []');
									}
									else if(this['Describe how measured (method)']) {
											if(this['Crop']){
													Common.print(response).textPlain('namespace: ' + Common.translate(this['Crop'], isoLang)+'Method');
											}else if (crop!=''){
													Common.print(response).textPlain('namespace: ' + crop+'Method');
											}
											Common.print(response).textPlain('def: "' + Common.translate(this['Describe how measured (method)'], isoLang) + '" []');
											...
											Common.print(response).textPlain('def: "' + Common.translate(this['Method description'], isoLang) + '" []');
									}

									///synonyms
									if(this['Synonyms (separate by commas)']) {
											if(Common.translate(this['Synonyms (separate by commas)'], isoLang).split(",").length > 1){//several syn
													var synTab = Common.translate(this['Synonyms (separate by commas)'], isoLang).split(',');
													for(var j=0; j<synTab.length; j++) {
															Common.print(response).textPlain('synonym: "' + synTab[j] + '" EXACT []');                            }
											}else{
													Common.print(response).textPlain('synonym: "' + Common.translate(this['Synonyms (separate by commas)'], isoLang) + '" EXACT []');
											}
									} else if(this['Trait synonyms']) {
											if(Common.translate(this['Trait synonyms'], isoLang).split(",").length > 1){//several syn
													var synTab = Common.translate(this['Trait synonyms'], isoLang).split(',');
													for(var j=0; j<synTab.length; j++) {
															Common.print(response).textPlain('synonym: "' + synTab[j] + '" EXACT []');                            }
											}else{
													Common.print(response).textPlain('synonym: "' + Common.translate(this['Trait synonyms'], isoLang) + '" EXACT []');
											}
									} else if(this['Variable synonyms']) {
											if(Common.translate(this['Variable synonyms'], isoLang).split(",").length > 1){//several syn
													var synTab = Common.translate(this['Variable synonyms'], isoLang).split(',');
													for(var j=0; j<synTab.length; j++) {
															Common.print(response).textPlain('synonym: "' + synTab[j] + '" EXACT []');                            }
											}else{
													Common.print(response).textPlain('synonym: "' + Common.translate(this['Variable synonyms'], isoLang) + '" EXACT []');
											}
									}

									//abbrev
									if(this['Abbreviated name']) {
											Common.print(response).textPlain('synonym: "' + Common.translate(this['Abbreviated name'], isoLang) + '" EXACT []');
									} else if(this['Trait abbreviation']) {
											Common.print(response).textPlain('synonym: "' + Common.translate(this['Trait abbreviation'], isoLang) + '" EXACT []');
									} else if(this['Trait abbreviation synonym']) {
											Common.print(response).textPlain('synonym: "' + Common.translate(this['Trait abbreviation synonym'], isoLang) + '" EXACT []');
									} else if(this['Main trait abbreviation']) {
											Common.print(response).textPlain('synonym: "' + Common.translate(this['Main trait abbreviation'], isoLang) + '" EXACT []');
									} else if(this['Alternative trait abbreviations']) {
											Common.print(response).textPlain('synonym: "' + Common.translate(this['Alternative trait abbreviations'], isoLang) + '" EXACT []');
									}
									///scales
									if(this.relationship) {
											if(this.relationship == 'scale_of'){
													if(this['Crop']){
															Common.print(response).textPlain('namespace: ' + Common.translate(this['Crop'], isoLang)+'Scale');
													}else if (crop!=''){
													Common.print(response).textPlain('namespace: ' + crop+'Scale');
													}

													if(typeof this.parent != 'string'){
															for(var i in this.parent) {
																	Common.print(response).textPlain('relationship: ' + this.relationship + ' ' + r.encodeID(this.parent[i]));
															}
													}else{
															Common.print(response).textPlain('relationship: ' + this.relationship + ' ' + r.encodeID(this.parent));
													}

													var type = Common.translate(this['Type of Measure (Continuous, Discrete or Categorical)'], isoLang)
																					|| Common.translate(this['Scale class'], isoLang);
													if( type == 'Categorical' || type == 'Ordinal' || type == 'Nominal') { // it's categorical
															for(var i in this) {
																	if(i.indexOf('For Categorical') == 0 || i.indexOf('Category') == 0) { // starts with
																			...
																			var category = Common.translate(this[i], isoLang);
																			var nameId = category.split("=")[1];
																			var synId = category.split("=")[0];

																			Common.print(response).text('');
																			Common.print(response).textPlain('[Term]');
																			Common.print(response).textPlain('id: ' + r.encodeID(this.id) + '/' + categoryId);
																			Common.print(response).textPlain('name: ' + nameId);
																			if(this['Crop']){
																					Common.print(response).textPlain('namespace: ' + Common.translate(this['Crop'], isoLang)+'Scale');
																			}else if (crop!=''){
																					Common.print(response).textPlain('namespace: ' + crop+'Scale');
																			}
																			Common.print(response).textPlain('synonym: "' + synId + '" EXACT []');
																			Common.print(response).textPlain('is_a: ' + r.encodeID(this.id));
																			Common.print(response).text('');
																	}
													...
													if(this['Crop']){
															Common.print(response).textPlain('namespace: ' + Common.translate(this['Crop'], isoLang)+'Variable');
													}
													Common.print(response).textPlain('relationship: ' + this.relationship + ' ' + r.encodeID(this.parent[0]));
													Common.print(response).textPlain('relationship: ' + this.relationship + ' ' + r.encodeID(this.parent[1]));
													Common.print(response).textPlain('relationship: ' + this.relationship + ' ' + r.encodeID(this.parent[2]));
													...
													Common.print(response).textPlain('is_a: ' + r.encodeID(this.parent));
									}

									Common.print(response).text('');
							});
					Common.print(response).text('[Typedef]')
					Common.print(response).text('id: method_of')
					Common.print(response).text('name: method_of')
					Common.print(response).text('is_transitive: true');
					Common.print(response).text('')
					Common.print(response).text('[Typedef]')
					Common.print(response).text('id: scale_of')
					Common.print(response).text('name: scale_of')
					Common.print(response).text('is_transitive: true');
					Common.print(response).text('')
					Common.print(response).text('[Typedef]')
					Common.print(response).text('id: variable_of')
					Common.print(response).text('name: variable_of')
	...
	"/terms/([^/]*)/(.*)" : {
			get: function(request, response, matches) {
					...
					if(info.indexOf("static-html") != -1) {
							...
							var skin = Common.renderIndex("skins/onto.html", {
								termid: termId
							});
					}
					//response.getWriter().println(skin);
					Common.print(response).html(skin);
					/*
	...
	"/search" : {
			get: function(request, response, matches) {
					...
					if(!q || q == "") return Common.print(response).json([], callback);
					...
					if(resultLength != words.length)
							return Common.print(response).json([], callback);
					...
					return Common.print(response).json(res, callback);
			}
	},
	*/
	"/login" : {
	...
	"/get-comments-onto" : {
			get: function(request, response) {
				...
				var term;
					...
					var termName = "";
					if ( term[0] ){
						var termName = Common.translate(term[0].getProperty("name"));
					}
	...
	"/next-id": {
			get: function(request, response) {
					...
					Common.print(response).json({"id": Common.pad(biggestInt+1, 7)});
			}
	},
	...
	"/get-term-parents/(.*)": {
		get: function(request, response, matches) {
			...
			function getParent(arr, untouched, branch, termId) {
				...
				for(var i=0; i<parentList.size(); i++) {
					...
					var o = {
						...
						relationship: Common.defaultRelationship(parentEntity.getProperty("relationship"))
					};
					if(i === 0) {
			...
			var o = {
				...
				name: "" + Common.object_key_replace("undefined", null, (name instanceof Text ? name.getValue() : name)),
				relationship: Common.defaultRelationship(termEntity.getProperty("relationship"))
			};
			var arr = [];
			var branch = [];
			branch.push(o);
			...
			Common.print(response).json(arr, request.getParameter("callback"));
			return arr;
		}
	},
	"/get-categories": {
			get: function(request, response) {
					...
					Common.print(response).json(cats);
			}
	},
	"/get-ontologies": {
		get: function(request, response) {
	...
	"/edit_profile": {
		post: function(request, response) {
			...
			if(!currUser){
				...
				Common.print(response).json(user);
				}
		}
	},
	"/users_admin": {
		get: function(request, response) {
			...
			if(!currUser){
				...
				var html = Common.renderIndex("skins/users_admin.html");
				Common.print(response).text(html);
			}
		},
		post: function(request, response) {
	...
	"/users/([a-zA-Z0-9_]+)": {
			get: function(request, response, matches) {
					...
					try {
							...
							Common.print(response).json(usermodel.out(userEntity));
					} catch (e) {
							response.sendError(response.SC_BAD_REQUEST, e);
					}
			}
	...
	"/feedback": {
			get: function(request, response) {
					var skin = Common.renderIndex("skins/feedback.html");
					response.getWriter().println(skin);
			}
	},
	"/annotation-tool": {
	...
	"/backup": {
			get: function(request, response) {
					...
					if(Common.isblank(kind))
							return Common.error(response, "Need kind");

					var currUser = auth.getUser(request);
					if(!currUser)
							return Common.error(response, "Not logged in");
					if(!auth.isAdmin(currUser))
							return Common.error(response, "Need to be an admin");
					...
					try {
							...
							return Common.print(response).json(arr);
					} catch (e) {
							return Common.error(response, e);
					}
			}
	},
	"/report": {
			get: function(request, response) {
				...
				if(Common.isblank(ontoId)) return Common.error(response, "Invalid parameter");
				...
				var result = [];
				terms.forEach(function(term){
					var item = googlestore.toJS(term);
					// obo_blob_key
					item.obo_blob_key = Common.object_key_replace("undefined", null, JSON.stringify(item.obo_blob_key));
					// xref
					item.xref = Common.object_key_replace("undefined", null, JSON.stringify(item.xref));
					// name
					item.name = Common.object_key_replace("undefined", null, JSON.stringify(item.name));
					// comment
					item.comment = Common.object_key_replace("undefined", null, JSON.stringify(item.comment));
					// is_a
					if(item.hasOwnProperty("is_a")) {
						item.is_a = Common.object_key_replace("undefined", null, JSON.stringify(item.is_a));
					}
					// synonym
					if(item.hasOwnProperty("synonym")) {
						item.synonym = Common.object_key_replace("undefined", null, JSON.stringify(item.synonym));
					}
					// def
					if(item.hasOwnProperty("def")) {
						item.def = Common.object_key_replace("undefined", null, JSON.stringify(item.def));
					}
					result.push(item);
				});
				...
				function addTo(obj, obj2, id) {
						...
						for(var i in order) {
								...
								if(obj2[newo]) {
										var t = ''+Common.translate(obj2[newo], language);
										if(!t) continue;
										obj[o] = t;
								} else {
										obj[o] = '';
						...
						if(id == 'scale' || id == 'obo') {
								for(var i in obj2) {
										...
										var t = ''+Common.translate(obj2[i], language);
										if(!t) continue;
										obj[i] = t;
								}
						}
			...
			post: function(request, response) {
					...
					if(excels[ontoId]) {
							...
							return Common.error(response, "Sorry there's no Excel version of this ontology");
					}
	...
	"/edit-profile": {
		post: function(req, res) {
			...
			if(!currUser) return Common.error(res, "Not logged in");
			if(Common.isblank(language)) return Common.error(res, "You need to insert a language");
	...
	"/add-parent": {
			get: function(req, res) {
					try {
							...
							if(Common.isblank(termId) || Common.isblank(parentId)) throw "Missing parameters";
							...
							return Common.error(res, e);
					}
			}
	},
	"/agtrials": {
			get: function(req, res) {
					var html = Common.renderIndex("skins/agtrials.html");
					Common.print(res).text(html);
			}
	},
	"/primary": {
			get: function(req, res) {
					var currUser = auth.getUser(req);
					if(!auth.isAdmin(currUser))
							return Common.error(res, "Not admin");

					var ids = req.getParameter("ids");

					var arr = JSON.parse(ids);

					Common.print(res).text(arr.length + " terms to be set primary<br><br>");
					...
					arr.forEach(function(id) {
							...
							googlestore.put(termEntity);
							Common.print(res).text(id + " was set");
					});
	...
	"/ibfieldbook": {
			get: function(req, res) {
					...
					if(!Common.isblank(ontologyId)) {
							if(obj[ontologyId])
									return Common.print(res).json(obj[ontologyId]);
					}

					if(html) {
							var html = Common.renderIndex("skins/ibfieldbook.html");
							return Common.print(res).text(html);
					}

					Common.print(res).json(obj);
			}
	},
	"/forgot-password": {
			get: function(req, res) {
					var html = Common.renderIndex("skins/forgot-password.html");
					Common.print(res).text(html);
			},
			post: function(req, res) {
					var data = {};
					...
					var html = Common.renderIndex("skins/forgot-password.html", data);
					Common.print(res).text(html);
			}
	},
	"/reset-password": {
			get: function(req, res) {
					...
					var html = Common.renderIndex("skins/reset-password.html", data);
					Common.print(res).text(html);
			},
			post: function(req, res) {
					var emailPar = req.getParameter('email');
	...
	"/get-term": {
				get: function(request, response) {
						...
						for(var i in t) {
								if(i == termId) {
										return Common.print(response).json(t[i]);
								}
			...
			if (matches[1].match(/:/)){
				...
				traits.forEach( function(trait){
					...
					var object = {
											...
											"name": ""+ Common.translate(trait.getProperty("name")),
		...
		get: function(request, response) {
			...
			var out = {
				...
				"summary per crop":ret
						...
						Common.print(response).json(out, request.getParameter("callback"));
		}
	},
	"/ontos_stats_make_csv": {
		get: function(request, response) {
			try {
				...
				Common.print(response).textPlain(out);
			} catch(e) {
			}
		}
	},
