   ...
   */
		...
		value: function build_carousel() {
			...
			if (page == "ontology" || page == "term") {
				...
				$('<h1>', { "id": "page_subtitle" }).text(NAV.get_ontology_id())).append($('<h2>', { "id": "page_title" }).text(NAV.get_ontology_label())
				// )
				));
			} else {
						...
						$("#ontology_card").html($('<div>', { "class": "col s2" }).append($('<img>', { "class": "crop_pict responsive-img", "src": ontologies_data.ontology_picture }))).append($('<div>', { "class": "col s10" }).append($('<h1>', { "id": "page_subtitle" }).text(NAV.get_ontology_id())).append($('<h2>', { "id": "page_title" }).append(function () {
							...
							if (ontologies_data.lead_centers.length > 0) {
								...
								return $('<div>').append($.map(ontologies_data.lead_centers, function (v, k) {
									if (v.image) {
										var image = !STR.is_url(v.image) ? "common/img/" + v.image : v.image;
							...
							if (ontologies_data.partners.length > 0) {
								...
								return $('<div>').append($.map(ontologies_data.partners, function (v, k) {
									if (v.image) {
										var image = !STR.is_url(v.image) ? "common/img/" + v.image : v.image;
										return $('<a>', { "href": v.link, "target": "_blank" }).append($('<img>', { "src": "common/img/" + image }));
									}
								})).html();
							}
						})).append($('<td>', { "class": "center" }).append(function () {
