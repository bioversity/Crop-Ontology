	...
	*/
	build_pagination(options) {
		...
		let prev_page = (settings.current_page <= 1) ? 1 : settings.current_page - 1,
			...
			$left_arrow_btn = $('<li>', {"class": "prev_page_btn"}).append(
				$('<a>', {"href": "javascript:;"}).append(
					$('<span>', {"class": "fa fa-chevron-left"})
				)
			).click(() => {
				this.goto(settings.id, "prev", settings.content, settings.items);
			}),
			$right_arrow_btn = $('<li>', {"class": "next_page_btn"}).append(
				...
				this.goto(settings.id, "next", settings.content, settings.items);
			});
		...
		for (let p = 1; p <= settings.total_pages; p++) {
			...
			$news_pagination.append(
				$('<li>', {"class": pagelink_class + " page_" + p}).append(
					$('<a>', {"href": "javascript:;"}).text(p).click(() => { this.goto(settings.id, p, settings.content, settings.items); })
				)
			);
		}
	...
	goto (id, page, content, items) {
		let current_page = parseInt($("#" + id).data("current_page")),
